...
|---|---|---|---|
|No specific arguyment needed| preceeding argument `global variable_name` needed to initialize the variable value from a global parameter | such as `sum(), tuple(), abs()`| Requires a keyword `nonlocal` to emphasize that the variable is not local in a nested function.|
